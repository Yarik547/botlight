...
function rr(ctx, x, y, w, h, r) {  // ФІКС: закрив
...
export function renderSchedulePng({ schedule }) {
  ...
  for (let i = 0; i < groups.length; i++) {
    ...
    rr(ctx, x, y, cardW, cardH, 18); ctx.fill(); ctx.stroke();
    // Badge
    ...
    ctx.font = "700 18px Arial"; ctx.fillText("Електроенергії немає", x + 18, y + 92);
    const pairs = schedule?.[g] ?? [];
    if (pairs.length) {
      const lines = chunkPairs(pairs).slice(0, 6);
      ctx.fillStyle = "#111827"; ctx.font = "600 18px Arial";
      let ty = y + 125; for (const line of lines) { ctx.fillText(line, x + 18, ty); ty += 24; }
    } else {
